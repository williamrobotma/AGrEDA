...
#SBATCH --error=logs/ADDA/generated_test/gen_v1-%a-%N-%A.err

set -x

start=`date +%s`

CONFIG_FILES=$(sed -n "${SLURM_ARRAY_TASK_ID},$(($SLURM_ARRAY_TASK_ID+4))p" configs/generated/ADDA/a_list.txt)
